module m_swflow
  ...
  abstract interface
    ...
    ! ステップ末尾パス(gwflow/evap の後に呼ぶ。§26。持たない実装は
    ! 束縛しない=何もしない)
    subroutine procedure_swflow_post(p, g, s)
      use m_sysparam, only : t_sysparam
      use m_geoinfo, only : t_geoinfo
      use m_state, only : t_state
      type(t_sysparam), intent(in) :: p
...
!----------------------------------------------------------------------
! ステップ末尾パス(run_main が gwflow・evap の後に毎ステップ呼ぶ。
! post 口を持たない実装(STG)や σ 無効時は何もしない。§26)
!----------------------------------------------------------------------
subroutine m_swflow_post(sw, p, g, s)
  type(t_swflow), intent(in) :: sw
  type(t_sysparam), intent(in) :: p
  type(t_geoinfo), intent(in) :: g
  type(t_state), intent(inout) :: s
