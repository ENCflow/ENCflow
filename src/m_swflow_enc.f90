...
subroutine boundary_h(p, g, b, s, sx)
  ...
  do j = dcp%js, dcp%je
    do i = g%wx(1,j), g%wx(2,j)
      ...
      ! ため池の処理
      if (g%rsh(i,j) > 0.0 .and. g%rsh(i,j) > s%rsh(i,j)) then ! ため池に余力がある
        if (sx%h1(i,j) > (g%rsh(i,j) - s%rsh(i,j))) then       !   ため池があふれる
          sx%h1(i,j) = sx%h1(i,j) - (g%rsh(i,j) - s%rsh(i,j))  !     場の水深がため池の余力分だけ減る
          s%rsh(i,j) = g%rsh(i,j)                              !     ため池が満水になる
        else                                                   !   ため池があふれない
