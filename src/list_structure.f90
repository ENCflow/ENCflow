module list_structure
  ...
  type t_list_structure
    ...
    integer :: dam_lake(1:nstmax) = 0              ! >0: 湖沼参照(自前の湖面を持たず番号 m の
                                                   ...
                                                   !   引く=追加の放流工。m < n。参照先は
                                                   !   主放流を持つ湖沼であること)
    integer :: dam_gw(1:nstmax) = 0                ! 1: 湖水位を湖面セルの地下水頭に強制
                                                   !   (Dirichlet。fn_gwflow の側方流
                                                   !   f_gwlateral=1 が必須。貯留湖沼は
                                                   !   交換量を V に計上。§22 第3弾)
...
subroutine read_dam(un, list)
  ...
  integer :: dam_lake(1:nstmax), dam_gw(1:nstmax)
  ...
  namelist /list_struct_dam/ dam_in_cell, dam_out_cell, dam_hv, f_dam_mode, &
                             ...
                             dam_hmin, dam_hsur, dam_lake, dam_gw, fn_dam_map, &
