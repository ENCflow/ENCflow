...
module m_record
  ...
  ! ==================================================================
  ! フラックス測線は DDA 階段面方式(§23。2026-08-08 変更): セル番号で
  ...
  type t_flux
    ! DDA 階段面方式(§23。2026-08-08 変更): 両端セルの中心を結ぶ直線を
...
subroutine set_flux
  ...
  if (flxytype /= 0) then
    call par_abort("m_record: 測線の実座標指定(flxytype=1)は廃止されました。" &
                   ...
                   //"(測線は両端セル中心を結ぶ DDA セル列=階段面で定義されます。§23)")
  end if
...
subroutine m_record_flux(r, p, s)
  ...
  do ifl = 1, r%nfl
    ...
    ! DDA 階段面の積算(§23): 踏面=短手方向フラックス×係数 ct、
