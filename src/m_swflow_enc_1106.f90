module m_swflow_enc
  ...
  real :: p_adprunge_thresh != 1.8           ! threshold of adaptive Runge-Kutta

  ! 状態変数の構造体の宣言と定義
  type t_enc_status
    real, allocatable :: uv(:,:,:)   ! セル境界での流速(符合は中心セルから近傍セルに向かい正)
    real, allocatable :: uv1(:,:,:)   ! セル境界での流速(符合は中心セルから近傍セルに向かい正)
    real, allocatable :: mn(:,:,:)   ! セル境界での流量
    real, allocatable :: mn1(:,:,:)   ! セル境界での流量
...
subroutine init_enc_status(p, g, s, sx)
  ...
  allocate(sx%uv1(1:4,0:p%nx,0:p%ny), source = 0.0)
  allocate(sx%mn1(1:4,0:p%nx,0:p%ny), source = 0.0)
...
subroutine del_enc_status(sx)
  ...
  if (allocated(sx%uv1)) deallocate(sx%uv1)
...
subroutine calc_kth_momentum(p, g, s, sx, i, j, k, have_exflux, have_runge, have_error)
  ...
  if (f_adaptive_runge > 0) then
    if ((mne >= 0 .and. (mne1 > mne * maglim .or. mne1 < mne / maglim)) .or. &
      ...
      !call calc_kth_flux_newton(p, g, s, sx, uve, tae, i, j, k, in, jn, uve1, mne1)
    end if
  end if
  ...
  !if ((dh > 0 .and. s%h(i,j) - dhc < p%dd) .or. (dh < 0 .and. s%h(in,jn) - dhn < p%dd)) then
  !if ((dh > 0 .and. s%h(i,j) - p%dd < dhc) .or. (dh < 0 .and. s%h(in,jn) - p%dd < dhn)) then
    have_exflux = .true.
...
!----------------------------------------------------------------------
subroutine calc_kth_flux(p, g, s, sx, uve, tae, i, j, k, in, jn, f_runge, uve1, mne1)
  type(t_sysparam), intent(in) :: p
  type(t_geoinfo), intent(in) :: g
  type(t_state), intent(in) :: s
  type(t_enc_status), intent(in) :: sx
  real, intent(in) :: uve         ! セル境界での流速
  ...
  ! 水が無いセルからの流出はなし
  !if ((uve > 0 .and. s%h(i,j) <= p%dd) .or. uve < 0 .and. s%h(in,jn) <= p%dd) then
  !  uve1 = 0
  !  mne1 = 0
  !  return
  !end if
  ...
  ! セル境界での有効重力加速度を計算
  ge = p%gg                                 ! 重力加速度
  if (f_gravity_correction > 0) ge = adjust_ge(g, i, j, k, in, jn, vve, ge)
  ...
  do while (l <= 4)
    ...
    if (f_hcap_upwind > 0) he = adjust_he(uve, hc, hn, he)
    ...
    tfe = tfe - p%kk * p%cd / bbe * (1 - gve) * vve
    ...
    !   摩擦項を半陰解法で計算する
    !   どちらも中心セルから近傍セルに向かい正
    uve1 = (uve + (tae + tge) * dtl) / (1 - tfe * dtl) 
    mne1 = uve1 * he
...
! 中心セルi,jからk近傍セルへin,jnの流速uve1と単位幅流量mne1を計算する
!----------------------------------------------------------------------
subroutine calc_kth_flux_newton(p, g, s, sx, uve0, tae, i, j, k, in, jn, uve1, mne1)
  type(t_sysparam), intent(in) :: p
  type(t_geoinfo), intent(in) :: g
  type(t_state), intent(in) :: s
  type(t_enc_status), intent(in) :: sx
  ...
  real :: hc, hn
  integer :: l
    real :: tae0, tge0, tfe0, tfe1, vve0
    real :: a1, b1, c1, d1, aa1
    real :: a0, b0, c0, d0, aa0

  ! 水が無いセルからの流出はなし
  !if ((uve0 > 0 .and. s%h(i,j) <= p%dd) .or. uve0 < 0 .and. s%h(in,jn) <= p%dd) then
  !  uve1 = 0
  !  mne1 = 0
  !  return
  !end if
  ...
  tae0 = 1 / p%dt / lme                ! * (uve1 - uve)
  tge0 = ge / w8dr(k) * gve            ! * (hn - hc) + tg0e
  tfe0 = ge * rne**2 * gve             ! * uve * vve * m_ffactor_calc(hhe)
  tfe1 = p%kk * p%cd / bbe * (1 - gve) ! * uve * vve


  ! ニュートン法のループ
  ! なぜか4回以下も6回以上もダメ、5回だけOK
  do l = 1, 5
    ! セル境界での水深を求める
    he = ((hc + hn) + (hc0 + hn0)) / 4
    !he = (hc + hn) / 2
    ...
    ! セル境界での水深が上流側水深よりも深くならない様に調整
    if (f_hcap_upwind > 0) he = adjust_he(uve1, hc, hn, he)
    ...
    ! セル境界での重力項(符合は中心セルから近傍セルに向かい正)
    if (f_pressure_term > 0) then
      tge = tg0e + tge0 * ((hn - hc) + (hn0 - hc0)) / 2
      !tge = tg0e + tge0 * (hn - hc)
    end if

    ! セル境界での摩擦項
    !   摩擦項は半陰解法で計算するため、次元が他の項と異なる(値は常に正)
    !tfe = tfe0 * (vve + vve0) / 2 * m_ffactor_calc(hhe)
    tfe = tfe0 * vve * m_ffactor_calc(hhe)

    ! セル境界での抗力項
    !   摩擦項と一緒に半陰解法で計算するため、次元が他の項と異なる(値は常に正)
    !tfe = tfe + tfe1 * (vve + vve0) / 2
    tfe = tfe + tfe1 * vve

    ! セル境界での流速(符合は中心セルから近傍セルに向かい正)を更新
    a1 = 1 / p%dt / lme            ! 局所項1次
    a0 = -uve0 / p%dt / lme        ! 局所項0次
    b1 = 0                         ! 移流項1次
    b0 = tae                       ! 移流項0次
    c1 = 0                         ! 重力項1次
    c0 = tge                       ! 重力項0次
    ...
    ! ニュートン法の次段のために流速の絶対値と水深を更新
    !   移流項はループに含まれないため精度は陽的オイラーのまま
    block
      integer :: kk
      real :: mnec, mnen
      real :: dhc, dhn
      ...
      do kk = 1, 8
        ...
        if (kk == 9 - k) mnen = -mne1     ! 近傍セルの流出量は中心セルの流出量の逆符号
        ! 仮の水深を更新
        dhc = mnec * mn2dh(kk) / g%gv(i,j)
        dhn = mnen * mn2dh(kk) / g%gv(in,jn)
        hc = hc - dhc
        hn = hn - dhn
        hc = max(hc, 0.0)
        hn = max(hn, 0.0)
...
!--------------------------------------------------------------------
function adjust_he(uve, hc, hn, he) result(he1)
  real, intent(in) :: uve    ! セル境界上の流速
  real, intent(in) :: hc     ! 中心セルの水深
  real, intent(in) :: hn     ! 近傍セルの水深
  real, intent(in) :: he     ! セル境界上の水深
  real :: he1                ! 調整後のセル境界上の水深
  if (uve > 0) then      ! 中心セルが上流側
    he1 = min(he, hc)      !   中心セルの水深より深くならないように
  else if (uve < 0) then ! 近傍セルが上流側
    he1 = min(he, hn)      !   近傍セルの水深より深くならないように
  else
    he1 = he
  end if
end function
...
!--------------------------------------------------------------------
function adjust_ge(g, i, j, k, in, jn, vve, ge) result(ge1)
  type(t_geoinfo), intent(in) :: g  ! 地理情報
  integer, intent(in) :: i, j, k    ! 中心セルのインデックスと方位
  integer, intent(in) :: in, jn     ! 近傍セルのインデックス
  real, intent(in) :: vve           ! セル境界上の流速の絶対値
  real, intent(in) :: ge            ! 調整前の重力加速度
  real :: ge1                       ! 有効重力加速度
  if (vve > 0) then
    ge1 = ge * w8dr2(k) / (w8dr2(k) + (g%z(in,jn) - g%z(i,j))**2)
  else
    ge1 = ge
  end if
end function
...
subroutine continuous(p, g, s, sx)
  ...
  !$omp parallel do private(i, j, k, in, jn, ie, je, uv1, mn1, dh)
  do j = g%wy(1), g%wy(2)
    do i = g%wx(1,j), g%wx(2,j)
      if (g%sw(i,j) > 0) cycle
      if (g%x(i,j) <= 0) cycle
      ...
      do k = 1, 8
        ...
        uv1 = sign_e(k) * sx%uv1(ke(k),ie,je)
        mn1 = sign_e(k) * sx%mn1(ke(k),ie,je)
        ! 水深の減少量(m)に換算
...
subroutine complete(p, g, s, sx)
  ...
  do j = g%wy(1), g%wy(2)
    do i = g%wx(1,j), g%wx(2,j)
      if (g%x(i,j) <= 0) cycle
      sx%uv(:,i,j) = sx%uv1(:,i,j)
      sx%mn(:,i,j) = sx%mn1(:,i,j)
      s%h(i,j) = sx%h1(i,j)
