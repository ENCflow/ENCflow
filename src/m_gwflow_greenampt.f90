module m_gwflow_greenampt
  ...
  !
  ! 【F ≡ s%hg の等価性(重要)】
  !   排水・側方流出を持たない現段階では、累積浸透深 F は柱状換算の
  !   地下貯留 s%hg と恒等なので、F のモデル私有配列は持たず s%hg を
  !   そのまま使う(リスタートも m_state の hg 保存で自動的に整合。
  !   契約5の私有保存は不要)。側方流動(m_gwflow_lateral_*)の導入後は
  !   s%hg が側方でも変化して恒等が崩れるため、F を私有状態に昇格し
  !   gwflow_greenampt.dat で save/restore すること。
  !
  ! 【F→0 の特異性】
