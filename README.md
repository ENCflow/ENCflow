...
- **他のモデルと組み合わせる** — BMI は、世界中の水文・地形・雪氷などの
  モデルが採用している「モデル同士をつなぐ共通の差し込み口」です。
  ENCflow を部品のひとつとして、降雨流出モデルの出力を受けて氾濫を
  解く、地形進化モデルと地形をやり取りしながら「氾濫が地形を変え、
  変わった地形の上を次の氾濫が流れる」実験を組む、観測データを計算の
  途中で取り込む(データ同化)——そうした異分野のモデルをまたぐ
  結合シミュレーションへの扉が開きます。
