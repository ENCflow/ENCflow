...
入力体系のまま、必要なプロセスを1つずつ足すだけです。まず簡潔な
モデルで「このプロセスは効きそうか」を見積もり、効くと分かってから
必要に応じて各分野の専門モデルへ進む。ENCflow は、その**最初の一歩の
ための共通基盤**でもあります(下げるのは「試す」障壁です。結果の
解釈には、その分野の知識が引き続き必要です)。
