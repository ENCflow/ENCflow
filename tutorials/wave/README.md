...
このチュートリアルの主役は、各 Step のパラメータファイル
(`param_step1.txt` など)です。**コマンドを実行するだけで先へ進まず、
その都度パラメータファイルを開いて、中身を本文と見比べながら**
進めてください。Windows(WSL)の方は、エクスプローラーで
`tutorials/wave` を開いておき、パラメータファイルをメモ帳などで
開くのが手軽です(開き方は
