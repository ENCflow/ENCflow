...
- (追記 2026-08-14)機能面に加えて利用者文書の整備が一巡した:
  ユーザーガイド 17 章+全パラメータ索引(370 項目)、全機能の
  注釈付き namelist 見本(examples/List_samples)、実行して学ぶ
  チュートリアル 2 件(最小例 wave・実地形 chichibu。実データの
  落とし穴 = 窪地除去・平坦区間由来の流量振動まで教材化)。
  導入コスト(独習可能性)の比較でも公開勢の上位に入る。一方で
  英語文書は README.en のみで、国際的には文書は依然弱点側。
  また比較優位の大半は「無償・全公開」の確定(ライセンス表記の
  復元。§34.3)が前提であることに注意。
