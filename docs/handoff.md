...
- **管路連続体層(都市下水道・岩盤水理)**: プロトタイプ**実装・検証済み
  ...
  **拡張方針を決定(2026-08-23。正本は developer.md §46.5。実証は
  ...
  (3) §46.5 の決定 8 項目の実装(**(1) 幾何換算式の記載・(2)
      fn_gwc_leak マップ化・(3) 陸側開放吐口・(4) サブサイクリング・
      (5) sy/slot_sy セル別化・(7) bot 負値許容・(8a) 機場 =
      f_pump_src=1・(8b) 海域セル吐口は実装・検証済み 2026-08-23 —
      §46.5 の実装記録(4 件)参照。残るは (6) 4 成分エッジマップ
      直接入力のみ**)。残:
      **サブサイクリング(m_glacier tick_flow が見本。静的 N の最小形
      から。既存ケースは N=1 でビット一致。実測で N ≳ 20〜50 が常態なら
      内側ループを超時間刻み法 RKL2 に置換するオプション — 古典 RK・
      予測子修正子は安定性目的では不採用。§46.5 (4))**→ sy・slot_sy のセル別化
      (必ずセットで。サブサイクリングが前提。一様=スカラーのビット
      一致 → マップ入力の 2 段)→ 4 成分エッジマップ直接入力+GIS
      前処理(utils/ 側。Python 可)→ fn_gwc_bot の負値許容(検査の
      分離。着手順は任意。ただし排出先の (8) と対)→ **海抜下・沿岸系の
      排出先: 機場 = struct_pump の取水源拡張(s%hgc。ため池取水と同じ
      多態の延長。f_gwpump 拡張は不採用 — 理由は §46.5 (8))と海域セル
      吐口(受け水頭=潮位+フラップ。culv_flap 前例)**。
      サブサイクリングは gwflow_lateral・layer2・creep・saltwater 側方への
      横展開候補(§46.5「(4) の横展開」)
...
- **淡塩2層(鋭利界面)モデル**: 段 1〜3 **実装・検証済み 2026-08-18**
  ...
  確認。残:
  (1) test/salt の暫定 reference の目視承認(プール実験)
