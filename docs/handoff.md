...
1m. **積雪・融雪 m_snow(§31。2026-08-10 実装)の残項目**
   ...
   - 氷河: 多年性 SWE の涵養・消耗(度日法の延長)→ 氷体流動 SIA
...
1n. **長期地形変動(§32。2026-08-10 実装)の残項目**
   ...
   - 長期実行の実証ラン(代表年×morfac×リスタート連鎖で 1 kyr 規模の
     平衡地形・土層厚分布の定常性確認)
   - 氷河地形(SIA 氷体流動+氷河侵食則。§32 の将来枠 L4)

1o. **海岸堤防(仮想壁面の海岸応用。2026-08-10 設計合意・未実装)**
   - §17 の規約を「仮想壁の天端は水域セル(rw/sw)が持ち、水域と陸の
     間の全エッジ(対角含む)に壁」へ一般化し、**海側(sw)セル持ち**で
     実装する(河川堤防と同型 = bank_wall フックの隣に seawall 判定を
     追加。越流=本間公式・モード機構を流用)
   - 指定は河川堤防と完全独立: fn_seawall(海側セル位置の天端分布、
     -900 以下はなし)/ seawall0(一律値)/ f_seawall_datum
     (**既定=絶対標高**。海岸の天端高は T.P.+○m で公表される実態。
     陸側地盤基準はオプション、f_bank_aggr 相当の集約を流用)/
     f_seawall_mode(0:越流のみ, 1:フラップ=陸閘・水門相当,
     2:強制排水=排水機場相当)。置き場は &list_tide(海水位の供給者と
     対。河川堤防が fn_channel に居るのと対称)
   - 検証は §17 と同型(無効時ビット一致・越流開始水位の解析検定・
     np ラダー)+潮位往復での越流/遮断の切替確認
