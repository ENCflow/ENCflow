...
- **管路連続体層(都市下水道・岩盤水理)**: プロトタイプ**実装・検証済み
  ...
  **拡張方針を決定(2026-08-23。正本は developer.md §46.5。実証は
  ...
  (3) §46.5 の決定 8 項目の実装(**(7) bot 負値許容・(8a) 機場 =
      f_pump_src=1・(8b) 海域セル吐口 fn_gwc_outfall は実装・検証済み
      2026-08-23 — §46.5 の実装記録参照**)。残: 幾何換算式の
      users_guide 記載(コード不変)→ fn_gwc_leak マップ化 →
      開放吐口(陸側の自由流出)→
