...
- **ライセンス確定後の README 追記(忘れないように)**: (1)「選ぶ理由」の
  ...
  DOI バッジを追加、CITATION.cff に DOI を反映。(3)「第三者の著作物を
  含まない」の主張は現状**計算コードに限定**している — examples の
  実地形データ(chichibu / abukuma の bil)は出典・利用条件が
  リポジトリに未記載のため、出典表記を整備するまでリポジトリ全体には
  広げないこと(2026-08-19 判断)。
