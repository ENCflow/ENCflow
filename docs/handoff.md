...
1j. **土石流・地滑りモデル(f_debris/f_release/f_slide。2026-08-09 実装。
   ...
   - **文献照合(2026-08-16 更新)**: 江頭系はユーザー提供の文献
     (江頭1993 新砂防 46(2)・Morpho2DH ver.2 Solver Manual)で確定し、
     **f_dbres=2(江頭構成則)と f_dbed=2(江頭・芦田 E-D)を実装済み**
     (debris_plan.md §1・developer.md §28.5)。f_dbed=2 では第3領域式も
     不要。**残る未照合は高橋・Kanako 系(f_dbed=1)のみ**:
     (a) 勾配領域閾値 tanθ = 0.138 / 0.03、未成熟係数 6.7、上限 0.9C*
     (b) δe=0.0007・δd=0.05 の既定値と侵食・堆積速度式の形
     (c) 高橋(1977)ダイラタント則の sinα_i(a_i=0.042 は江頭1993で確定)
     Kanako 論文(Nakatani et al. 2008, IJECE 1(2))は照合済み(2026-08-16)
     だが閉じ式は含まれず、Takahashi & Nakagawa (1991) への参照だった。
     確定には **Takahashi & Nakagawa (1991), 砂防学会誌 44(3), 12-19** か
     高橋保『土石流の機構と対策』・砂防指針(土石流・流木対策編)解説が必要
