...
- **管路連続体層(都市下水道・岩盤水理)**: プロトタイプ**実装・検証済み
  ...
  (1) の承認をもって充足
- **淡塩2層(鋭利界面)モデル(設計検討段階・実装未着手)**: 地表水・
  浅層地下水に塩水層を導入し、SWI2 型の準静的近似で塩水くさび・
  海水浸入・淡水レンズ・塩害を扱う構想。合計 h/hg は不変+塩水層厚を
  下層固定で追加、密度は塩水層水頭 Φs = η + εζ にのみ現れる(運動
  方程式不変 = 方針 §0「SWE+1方程式拡散が上限」の内側)。設計正本は
  **docs/swi_plan.md**(検証解析解・実装順序・未決事項 α/海側境界/
  逆成層規則を整理済み)。優先順位は管路連続体層の残作業より後
