...
- **溶岩流モジュール(m_lavaflow。2026-08-23 起案・未実装)**:
  設計文書 docs/lava_plan.md を起案済み(独立モジュール・深さ平均
  Bingham 粘性重力流=拡散1方程式・段階1は等温+レート固化 → z、
  噴火口はモジュール私有ソースのセル群+噴出率時系列、レオロジーは
  ラン全体1組)。討議の合意: 粘度の噴火口別指定はしない(粘度差は
  段階2の温度従属+噴火口毎の噴出温度で表現)。固化 → z により
  冷却後の降雨・洪水の連鎖は追加実装なしで同一ラン。実装は
  lava_plan.md §10 の L2 から。
...
- **管路連続体層(都市下水道・岩盤水理)**: プロトタイプ**実装・検証済み
  ...
  (4) 幹線ハイブリッド(埋設河道。plan §4.4)、SWMM 参照解(確実層)
