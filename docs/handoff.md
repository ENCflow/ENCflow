...
1g. **潮位境界 m_tide(2026-08-08 実装。仕様は developer.md §23)の残検証**
   ...
   - test/tide の reference は目視承認済み(2026-08-24)= 確定
...
1j. **土石流・地滑りモデル(f_debris/f_release/f_slide。2026-08-09 実装。
   ...
   - **文献照合(2026-08-16 完了)**: 提供文献4本で主要な閉じ式は
     ...
     **上限 0.9C* は国総研資料 904(指針解説)で出典確定(2026-08-21。
     developer.md §50.5)**。指針の下限 Cd ≥ 0.3 は計画ピーク流量用の
     安全側クリップとして不採用(理由は同 §)。
     **残るのは 0.138/0.03・係数 6.7・δe=0.0007 既定の出典のみ**
     (指針解説にも現れない — 高橋成書(未成熟土石流平衡濃度式)の
     系譜。f_dbed=2/3 が文献照合済みのため優先度低)
   ...
   - **火山流動の等価流体モード(2026-08-16 実装済み)**: f_dbed=0 +
     ...
     (developer.md §28.8、debris_plan.md §5)。
     **雪崩(流れ型)への適用ガイドを整備済み(2026-08-21。
     users_guide/geomorph.md)。走路の雪連行も f_dbed=4(速度比例
     連行則)として実装済み(2026-08-21。test/avalanche。§28.8)**。
     雪崩固有の残り: SWE→連行可能層(sd)の自動変換(現状は前処理)、
     乾式流動(疑似間隙水の解消 = Phase 2 案c と共通)、実事例
     ランアウト・H/L との突合。残作業:
...
1k. **流木モジュール(fn_driftwood。2026-08-21 実装。developer.md §50、
   ...
   - **文献照合: 完了(2026-08-21)**。
     Braudrick & Grant (2000)(§50.4)— 喫水を式(23)(6)の厳密解に
     置換・確定、長さ非依存の抽象化と接地判定の裏付け、再流動の実測
     目安(平行・rootwad 付きは d/d_b ≈ 1.2)を記録。
     国総研資料 904・905(§50.5)— 侵食連行(全量流失)が指針の
     発生量算出と同じ考え方であることを確認、**水理的流失
     dw_hrec/vrec/wrec は指針に水理閾値式が存在せず「校正パラメータ
     として確定」**、実務値(dw_dlog ≈ 平均胸高直径、wst 実績包絡値、
     流木流出率 0.8〜0.9)をガイドへ反映。
     残るのは校正パラメータの実流域での当たり値のみ(感度解析の領域)
   - 合意済みの将来拡張(driftwood_plan.md §6): 多クラス化(径級・
     樹種)、侵食増分比例の連行、境界流入の流木時系列(cbin 経路)、
     測線流木フラックス(m_record)、抵抗則への算入、動的閉塞則
     (第1段では対象外と明言 — シナリオ与件の2ケース比較で運用)
   - 実流域ケースでの感度解析例(閾値・レートの当たり値)と
     ユーザーガイド例題化(遊砂地あり/なしの流木捕捉比較)
...
1k. **土壌雨量指数 m_swi(fn_swi。2026-08-20 実装済み。developer.md §49)の残項目**
   - タンク定数の原典照合(石原・小葉竹1979 / 岡田ほか2001 測候時報68。
     実装値はコード内【要文献照合】マーク付き。文献入手後に確定)
   - 原雨量スナップショット方式への拡張(現状は専用ラン=他モジュール
     排他。スナップショットにすれば物理モデル(hg 飽和度・Fs)との
     同走比較が可能になる — §49.1)
   - 警戒判定の後処理例(60分積算雨量とのスネークライン、CL 線比較)の
     ユーティリティ化または ai_guide 教材化
   - ifx / PREC=single での確認(gfortran のみ実施済み)
...
- **溶岩流モジュール(m_lavaflow。段階1 実装・検証済み 2026-08-23)**:
  ...
  →ラハール生成の結合(1j-(iv) の融雪型泥流と共通の抽象化未設計)、
  (iv) 実溶岩流事例(到達範囲・流下時間)との突合。

- **ダム・湖沼への拡張(&list_struct_dam の一般化)**: 第1弾
  (fn_dam_map・自動線形 HV・水位固定湖沼・素通し・dam_area 廃止)、
  第2弾(複数の放流工 dam_lake)、第3弾(湖水位の地下水頭強制
  dam_gw)まで **2026-08-20〜21 実装・gfortran 検証済み**(決定の
  正本は developer.md §22「ダム・湖沼への一般化」)。
  残: (a) ifx・PREC=single での同検証、(b) リスタート往復(新規状態は
...
- **ライセンス: 完了(2026-08-21)**。Apache-2.0 設置、CITATION.cff
  ...
  (Concept DOI 10.5281/zenodo.22042847)、README DOI バッジまで実施済み
...
- **乾式斜面侵食 f_splash の続き(developer.md §48.2。2026-08-20)**:
  ...
  積み残し: (a) Phase 2 = 雨滴項の風向依存 Φ_A(θ_wind)、κ 評価の平滑化、
  新島 1m DEM での Φ_B 較正、(b) Phase 3 = 容量式の堆積(D8 経路)。
  白ママ対照は実施済み(test/splashslide。2026-08-20)。詳細文書は
  docs/splash_plan.md。研究文脈(新島の谷形成仮説・論文化)は
  利用者側の計画であり本リポジトリには持ち込まない。

- **小拡張候補(users_guide/usecases.md「現時点では未対応(計画中)」に
  対応。2026-08-18 の見落とし点検で列挙)**: いずれも既存の枠組みに乗る:
  (1) ~~井戸揚水・地下水取水のシンク~~ **実装済み 2026-08-18**
      (m_gwflow_pump。f_gwpump + &list_gwflow_pump。セル集合+時系列、
      層選択(土層/層2)、貯留キャップ、淡塩併用時は淡水優先。
      揚水誘引の海水浸入を確認済み。developer.md §16.3)
  (2) 風応力項(吹送流・wind setup。m_meteo の風速枠〔エネルギー収支法
      用に計画済み〕と共用)
  (3) ~~浸透パラメータのマップ化~~ **実装済み 2026-08-18**
      (fn_gw_ksv / fn_gw_psif。カーネルは常に帯配列参照で分岐なし、
      一様指定はスカラー値充填 = 従来とビット一致。マップの K_sv=0 は
      不浸透=完全舗装。developer.md §16 の追記参照)
  (4) ~~凍土による浸透抑制~~ **実装済み 2026-08-18**
      (m_gwflow_frost。f_gwfrost + &list_gwflow_frost。度日ベースの
      凍結指数→鉛直浸透能の線形低減、積雪断熱・融解効率・初期凍結
      オプション、私有 save。developer.md §16.4)
  (5) 田んぼダムの定型化(貯留+落水口オリフィスの組み合わせの標準
      レシピ化。要検討)

- **管路連続体層(都市下水道・岩盤水理)**: プロトタイプ**実装・検証済み
  2026-08-18**(m_gwflow_conduit + conduit_core。単一モジュール+直交
  ...
  **拡張方針を決定(2026-08-23。正本は developer.md §46.5。実証は
  ...
  昇格は下水道×岩盤の併用専用のまま)。残:
  (1) test/conduit・test/coastal_drain・test/sewer_wq の暫定 reference の
      目視承認 — **済(2026-08-24)= 3 件とも確定。これにより
      プロトタイプ実証は閉じた**
  (2) 閾値切替流束則 f_gwc_fluxlaw=3(CFPM2 型。Darcy⇄D-W)
  (3) §46.5 の決定 8 項目の実装: **(1)〜(5)・(7)・(8a)(8b) は実装・
      検証済み 2026-08-23(§46.5 の実装記録 4 件を参照)**。残るは
      (6) のみで、**2026-08-23 改定によりエッジマップ直接入力は不採用 —
      「通水能密度の異方セル量化 fn_gwc_cnd_x/y +管路遮断壁ラスタ
      (仮想壁面の型)」に差し替え**、実施は実都市フェーズ(GIS 前処理・
      SWMM 参照解 = plan 段6)と一括。水質連携(§30 の管路連携)より
      後で良い(交換項/側方係数で直交)。
      サブサイクリングの gwflow_lateral・layer2・creep・saltwater 側方への
      横展開と、N ≳ 20〜50 常態時の RKL2 置換は候補のまま
      (§46.5「(4) の横展開」・(4) 本文)
  (4) 埋設河道(断面幾何つき準1次元)の新設判断は SWMM 参照解比較
      (plan §10.5 段4・段6)で連続体セル列の不足が実証されてから。
      SWMM 参照解(確実層)の構築は残
  (5) 疑似スロット幅・eps_h の感度実験と標準値(plan §10.7。
      sewer_hybrid で「緩い slot_sy = 被圧側の幻の貯留 → 氾濫過小評価」
      と「幹線通水能が dt 上界を支配」を実測済み — §46.5 の数値特性)
- **淡塩2層(鋭利界面)モデル**: 段 1〜3 **実装・検証済み 2026-08-18**
  (m_saltwater + fn_salt。単一モジュール+完全加算方式。developer.md
  §47、設計は docs/swi_plan.md)。模式実験 test/salt でプール仕切りの
  解析終状態(10/21 一致・H 厳密不変・体積保存)と海水浸入の水頭平衡
  (hg → 0.500)、np=1,2,4 バイト一致・-fcheck np=2・リスタート往復を
  確認。残:
  (1) test/salt の暫定 reference の目視承認 — 済(2026-08-24)= 確定
  (2) 段 4: 蒸発・浸透の淡塩配分(塩害評価。fxg の读取分担を wq と調整)
  (3) 解析解ベンチ: Glover 定常くさび・円形島の淡水レンズ(要涵養)
  (4) α の感度実験(河口くさび。Keulegan 比較)と既定値の確定
  (5) 分割誤差の恒久対策が要る場合: lateral_core の 2 zone 一体求解版
      (等価リファクタ。§47.2)
  (6) 層2・管路層への塩水拡張、wq 併用の塩分濃度診断
...
1l. **水質(負荷流出)m_wq(§30。2026-08-09 実装)の残項目**
   ...
   - W3(地下横輸送+湧出還元+遅延化係数 wq_rg): **実装済み
     (2026-08-26。§30.5 / wq_metal_plan.md。実装は記録契約でなく
     lateral_core への同伴輸送の畳み込み)**。
     test/sewer_wq の wq.csv reference は許可を得て更新済み
     (2026-08-26。差は全行 0 の seep_g 列挿入のみ・既存列と Log は
     ビット一致を確認)
   - 貯水池内混合(M/V 完全混合)+ため池(rscap)吸収の質量同伴:
     **実装済み(2026-08-26。§30.7 / test/damwq)**。残: 池内沈降
     (必要が出たら M の一次沈降係数)、湖沼参照(lref)複数放流工の
     実ケース検証、wq.csv 4 列追加に伴う gwseep/kdpart/sewer_wq の
     reference 更新(差は全行 0 の列挿入のみ・機械確認済み —
     人間の確認後に ./Run.sh -u)
   - 温度補正 k(T)=k20·θ^(T−20)(m_meteo の気温を水温プロキシに)
   - 粒子吸着系の Kd 平衡二相分配(K1): **実装済み(2026-08-26。
     wq_kd / §30.6 / test/kdpart。浸透 fd・沈降 fp×浮遊砂 wf)**。
     残(K2): E-D フラックスとの厳密結合(記録契約+河床交換層の
     金属/砂比)— K1 の実流域適用で不足が示されてから(wq_metal_plan §5)
