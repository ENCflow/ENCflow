...
- **管路連続体層(都市下水道・岩盤水理。設計検討段階)**: 下水道網・
  岩盤亀裂網を「非線形流束則つき等価被圧連続体層」として gwflow 系の
  第 3 の加算的レイヤ(f_gwconduit)で表す構想。用途別にモジュールを
  分けず単一モジュール+直交スイッチ(地表交換・層間交換・流束則)と
  する案を推奨(docs/gwconduit_plan.md §10。設計合意後に実装。
  科研費基盤(B)申請〔〜2026-09-17〕前にプロトタイプ+模式実験 1 件が
  目標。実装順序は同 §10.5)
