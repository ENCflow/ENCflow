...
1l. **水質(負荷流出)m_wq(§30。2026-08-09 実装)の残項目**
   ...
   - 2層 gwflow(風化基岩層)とその cg2(§30 の議論参照。逓減解析+
     トレーサ滞留時間でパラメータが分離同定できる)
   - L-Q 式(流量依存負荷): 表面蓄積プール+洗い出し(buildup-washoff)
     方式が本命(§30 の議論参照)。土地利用別原単位は fn_wq_map、
     降雨中濃度(湿性沈着)は wq_rain_conc/series で実装済(2026-08-09)。
     樹冠遮断分の沈着の地表還元(樹冠洗い落とし)は未対応
