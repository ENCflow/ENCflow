...
1d. **河川堤防のモデル化(セル間仮想壁面)— 設計議論から着手(未着手)**
   (2026-08-06 に依頼を受領。実装可否・分類名称も含めた検討から始める。
    実装前に設計合意を得ること)
   - 目的: セル間に仮想的な壁面を置いて河川堤防をモデル化する
   - 入力(既存の河道マスク rw の位置に追加する2つの分布):
     (1) 河床深さ分布 — 河道セルの標高を指定深さだけ掘り込む
         (既存の depth_rw(河道一律の掘り込み)との関係整理が必要)
     (2) 堤防高さ分布 — 河道幅1セルなら右岸左岸同値、2セル以上なら
         右岸左岸で別の値を与えられるようにする
   - 堤防高さの定義はフラグで3択切替: 河床からの高さ /
     非河道側セル標高からの高さ / 絶対標高。
     フラグを領域一様にするか河道セルごとに指定できるようにするかは
     発注者も迷っており要議論
   - 堤防の働き(堤内地=非河道セルから河道への排水機能)もフラグで
     3モード切替(名称は仮。良否含め要検討):
     ・強制排水: 非河道→河道は常に段落ち
     ・一方向排水: 非河道→河道は壁なしで通常の流れ計算、
       河道→非河道は壁の高さまで不透過・壁を越えたら段落ち
     ・自然排水: 双方向とも壁の高さまで不透過、壁を越えたら段落ち
   - 検討の観点(次チャットで): 段落ち・不透過壁の既存機構との関係
     (m_swflow_enc の boundary/wall 系、user フックの wave_solid_wall /
     wave_leaky_wall)、エッジ属性としての壁データの持ち方(セル間属性は
     エッジ配列になる=メモリと MPI エッジ規約への影響)、右岸左岸の
     判定方法、境界条件 §15 との整理、gwflow 側方流(§16.1)の
     河道セル透過との整合
