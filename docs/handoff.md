...
- **ダム・湖沼への拡張(&list_struct_dam の一般化)**: 第1弾
  (fn_dam_map・自動線形 HV・水位固定湖沼・素通し・dam_area 廃止)と
  第2弾(複数の放流工 dam_lake)は **2026-08-20 実装・gfortran
  検証済み**(決定の正本は developer.md §22「ダム・湖沼への一般化」)。
  残: (a) ifx・PREC=single での同検証、(b) リスタート往復(新規状態は
  ないので既存機構で閉じるはずだが実証)、
  (c) 第3弾=隣接セルの地下水位への湖水位強制(lake_plan.md §7)。
