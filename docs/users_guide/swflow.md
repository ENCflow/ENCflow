...
| f_check_cfl | 0 | 1 以上でクーラン数が 1 を超えたら状態を表示して停止(既定は続行) |
...
| パラメータ | 既定値 | 意味 |
|---|---|---|
| f_advection_tvd | 0 | 移流項に TVD スキームを使用 |
| p_adv_upwind_index | 0.5 | 移流項の風上化指数 |
| f_diffusion_term | 0 | 拡散項。0: なし(既定)、1: 定数粘性、2: ゼロ方程式モデル(ν = ν0 + α·u*·h) |
| p_diffusion_nu | 0.0 | 動粘性係数 ν0 (m²/s)(モデル 2 ではバックグラウンド粘性) |
