# 河道(&list_channel / channel_breach)

[ユーザーガイド目次へ](../users_guide.md)

セルサイズで解像しきれない河道の水理構造 — 堤防・サブグリッド河道幅・
断面形・破堤 — を扱います。`fn_channel` で有効化します。

**前提と役割分担**: 河道セルの指定(河道マスク `fn_rw`)と、セルの
地形・物性(河床の掘り込み `depth_rw`、河道粗度 `rn0_rw`)は
[地理情報の章](geoinfo.md)の設定です。本章はセル間の**通水構造**
(堤防=仮想壁面、河道幅、水理モード)を扱います。堤防の指定には
fn_rw が必須です。

## 堤防(仮想壁面)

河道セルと堤内地セルの境界に、セルを太らせない仮想の壁を立てます。
天端を超えた分は本間公式の堰越流で堤内地へあふれます。

分布ファイル `fn_bank` で高さを与えるときの注意:

- 高さは**河道セル**の位置に与えます(壁エッジは河道セルと堤内地
  セルの間の全エッジ — **対角を含む** — に立ち、天端には河道側セルの
  値が使われます)。海岸堤防 `fn_seawall` が陸側セル持ちなのと
  逆なので注意してください([地理情報の章](geoinfo.md))。
- **堤内地セルと角(対角)だけで接する河道セルにも高さを与えて
  ください**。値のない(−900 以下)河道セルのエッジには壁が立たず、
  ENC 格子は対角リンクでも水をやり取りするため、そこから堤内地へ
  漏水します。一律値 `bank0` は全河道セルに付与されるため、この
  心配はありません。
- 逆に、**余分に与えるのは無害**です。堤内地と接していない河道セルの
  値は使われず(壁は立たない)、河道外セルの値も無視されます。
  したがって河道沿いに**バッファ状に広く塗っておく**のが安全側で、
  上記の角接触の漏れも自動的に防げます。

```
&list_channel
  bank0 = 1.0                   ! 堤防高さの一律値(fn_bank と排他)
  f_bank_datum = 1              ! 高さの基準(下表)
/
```

| パラメータ | 既定値 | 意味 |
|---|---|---|
| fn_bank / bank0 | "" / — | 堤防高さ(分布 / 一律値。どちらか一方。分布の −900 以下は堤防なし) |
| f_bank_datum | 1 | 高さの基準。0: 河床(掘り込み後)基準、1: 堤内地セル標高基準(推奨)、2: 絶対標高 |
| f_bank_aggr | 0 | datum=1 での天端の集約。0: 隣接堤内地の平均(ノイズ抑制)、1: 最小(越流開始に忠実)、2: 最大(安全側) |
| f_bank_mode | 0 | 水理モード。0: 越流のみ(双方向とも天端まで不透過)、1: 樋門(逆止弁。堤内→河道のみ通す)、2: 強制排水(互換・実験用。実務の排水機場は [構造物の章](structure.md)のポンプを使用) |
| f_bank_opening | 1 | 斜め開口の通過幅を河道の法線エッジへ振り替える開口補正(幅1セル河道の過小通水を補正。0 は旧挙動との比較用) |

## サブグリッド河道幅

セルサイズ未満の川幅の通水能と貯留を、矩形断面のサブグリッド河道と
して表現します。`fn_width` の指定で有効化します。

**モデルの要点(重要な前提)**: 幅を有効にしたセルの物理量・状態量
(水深・水位・流量・貯留)は、**セル内の河道部分だけ**を表すように
なります。そのセル内の堤内地(非河道部分)は存在しないものとして
扱われ、河道と堤内地の水のやり取り(越流氾濫・氾濫水の戻り)は
**隣接する堤内地セルとの間で直接**行われます。RRI のように 1 セルに
斜面と河道の 2 つの水位を持つ方式とは異なる設計で、**セル内の堤内地
の貯留・浸水を表現できない**代わりに、川幅がセルサイズを跨いでも
モデルの切替が存在せず、**上流端の細流から下流端の大河まで単一の
定式でシームレスに解ける**ことを優先しています(この選択の経緯は
developer.md §18)。氾濫域はセル単位で解像されるので、河道セル自身
の中の氾濫まで解きたい区間は、セルサイズを川幅より細かくして解像
河道に移行してください。

```
  fn_width = "channel_width.txt"  ! 河道幅分布 (m。河道セルのみ有効。
                                  !   0 以下は幅情報なし=解像扱い)
```

- 堤防未指定なら「高さ 0・堤内地セル標高基準」の堤防が自動で有効化
  されます(河道幅は壁との併用が定式化の前提のため)。
- 幅がセルサイズ以上のセルは自動的に従来の解像河道(掘り込み+壁)と
  同じ扱いになります — 上流の細流から下流の大河まで1枚の幅データで
  つながります。
- このとき**幅の値そのものは結果に影響しません**。幅は常に
  「min(幅/セル寸法, 1)」型の係数としてだけ使われるため、セルサイズを
  超えた部分はどんな値でも(実河道幅でも 9999 のような便宜値でも)
  厳密に同じ結果になり、無害です。解像扱いにしたい区間には大きめの
  値を一括で与えて構いません(厳密には、河道の上流端・下流端の
  セルだけ平面積率の飽和にセルサイズの 2 倍が必要なので、確実を
  期すなら 2×セルサイズ以上、例えば 9999 を推奨)。
- **例外: 断面形 σ(p_sect_m > 0)と併用する場合**は、低水時の
  有効幅が「幅 × σ(h)」として幅の実値に依存するため、この
  「大きければ同じ」は厳密には成り立ちません(ただし過大な幅でも
  結果は解釈可能で大きくは破綻しません —
  [断面形の節](#断面形σ-則)の注記を参照)。
- `f_channel_advection = 0` で河道セルを含むエッジの移流項を落とせます
  (強い幅不均質下での安定化オプション。既定は 1 = 通常)。
- **適用条件**: 実河道がラスタより滑らかに走る解像度で使ってください。
  1セルの中で蛇行するような細かい川に使うと、河道長の過小評価により
  流量・流速が過大になります。
- **精度の限界(接続の曖昧さ)**: 河道はラスタ(河道マスク+幅)で
  表現され、河道セル同士の接続は「隣接していればつながる」という
  暗黙の規則で決まります。河道区間ごとの接続情報を明示的に持つ
  1次元河道網モデルと異なり、合流・分派の対応関係や、近接・並行する
  河道どうしの分離を厳密には指定できないため、意図しない接続や
  分断が生じ得ます。河道網の位相(どこがどこに流れ込むか)が結果を
  支配する用途では、この曖昧さが精度の限界になります。疑わしい
  箇所はセルサイズを細かくして接続・分離を解像してください。

## 断面形(σ 則)

低水路の水位低下・逓減の再現を改善する断面形の一般化です。

```
  p_sect_m = 0.5                ! 断面の形状指数(既定 0 = 矩形)
```

通水率を σ(h) = (h/D)^m とする単パラメータの断面形で、m = 0 が従来の
矩形、0.5 が放物線的、1 が三角形的な断面に相当します。遷移深さ D は
堤防有効セルで「天端−河床」、なければ掘り込み深さが使われ
(どちらも無いと p_sect_m > 0 はエラー)、h ≥ D では従来の矩形力学に
退化します。

σ の使用に河道幅(fn_width)は必須ではありません。**幅を指定して
いないセルでは、σ はセル全幅(=セルサイズ)の断面に適用**されます。
幅データを作らずに σ だけを有効化するのが最簡構成です。

**河道幅が不正確でも大きくは破綻しません**。σ 有効時の低水部は
有効幅 = 幅 × σ(h) として幅の値に依存しますが、実幅より過大な幅
(たとえばセルサイズや一律の便宜値)を与えた場合でも、低水時の流れは
「広い河床の中を細い澪筋が多数に分かれて流れる」複列断面と等価な
解釈が成り立ち、高水時(h ≥ D)は矩形力学に退化して実幅指定との差は
ほぼ消えます。「簡便なデータでもとりあえず動く」という本プログラムの
方針どおり、幅データの精度は段階的に上げれば十分です — 低水部の
水位・流速の再現を重視する区間から実幅に精緻化してください。

形状指数 m は現状、領域一様の 1 値です。上流の渓流と下流の大河で
断面形が異なるのは自然なので、必要が生じれば分布ファイルによる
セル別指定への拡張を検討します(将来拡張の候補)。

## 破堤(&list_channel_breach)

堤防の実効天端を時系列で下げ、決壊を表現します(堤防の有効化が
前提)。

```
&list_channel_breach
  br_cell(:,1) = 120, 45, 120, 44   ! サイト1: 河道セル ic,jc と堤内地セル il,jl
  br_series(:,1,1) = 0.0,  1.0      ! (時刻 min, 天端の残存割合 0〜1)
  br_series(:,2,1) = 60.0, 1.0      !   60 分までは無破堤
  br_series(:,3,1) = 70.0, 0.2      !   60〜70 分で 2 割まで決壊
  br_series(:,4,1) = 90.0, 0.0      !   90 分で堤内地盤高まで全壊
/
```

- サイトは「隣接する河道セルと堤内地セルの対」で、その境界エッジ
  1本の天端が 実効天端 = 堤内地盤高 + 割合×(天端 − 堤内地盤高) と
  変化します。幅1セルの河道でも堤内地セル側の指定で左右岸を区別
  できます。セル座標は**1 スタート**です(GIS の 0 スタートに注意。
  [座標系の章](coordinates.md))。
- 複数セル区間の決壊は複数サイトで指定します(決壊幅はエッジ単位)。

## 例題・関連

- 書式の実例: [examples/List_samples/list_channel.txt](../../examples/List_samples/list_channel.txt)
- 海岸堤防(fn_seawall)は同じ仮想壁面機構の海岸応用で、
  [地理情報の章](geoinfo.md)の設定です。
- 樋管・樋門を管路(断面・敷高・ゲート)として精緻に扱うなら
  [構造物の章](structure.md)のカルバートを使います(f_bank_mode=1 の
  逆止弁は簡便法)。
