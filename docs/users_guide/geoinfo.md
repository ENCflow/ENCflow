...
| rn0_rw | −1 | 河道セルの固定粗度。正値を指定すると、粗度の与え方(rn0 / ファイル / 土地利用)によらず**河道セルの粗度を上書き**します(掘り込み指定の有無と独立) |
...
マスクの作成は事前処理で行います。土地利用データ(fn_luse)がある
場合は、付属の `utils/lu2mask` で「指定した土地利用コードのセルを 1」
