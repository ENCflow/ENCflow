# 地理情報(&list_geoinfo)

[ユーザーガイド目次へ](../users_guide.md)

計算の土台となる格子・地形・粗度・各種マスクの設定です。唯一の
**必須機能**で、`fn_geoinfo`(既定 `"-"` = パラメータファイル内)で
読み込まれます。

最小構成は格子の定義だけです(チュートリアル wave がこの形です)。

```
&list_geoinfo
  lx = 100.0, ly = 100.0    ! 計算領域の大きさ (m)
  nx = 300, ny = 300        ! 計算格子数
/
```

実地形の計算では、ここに地形ファイル・粗度・マスク類を足していきます。

## 格子の定義

| パラメータ | 既定値 | 意味 |
|---|---|---|
| nx, ny | — | 格子数(必須) |
| dx, dy | — | 格子間隔 (m) |
| lx, ly | — | 領域の大きさ (m) |
| epsg | 0 | 格子の座標系の EPSG コード([座標系の章](coordinates.md)) |

各方向とも **nx と、dx か lx のどちらか**を指定します(dx と lx の
両方を書いた場合は整合が検査され、矛盾すると停止します)。bil 入力で
`.hdr` サイドカーがある場合は nx, ny, dx, dy が補完されるので
省略できます。

## 地形(地盤高)

| パラメータ | 既定値 | 意味 |
|---|---|---|
| f_ztype | 0 | 0: 固定値 z0、1: ファイル fn_z |
| z0 | 0 | 地盤高固定値 (m) |
| fn_z | "" | 地盤高分布ファイル |
| mag_z | 1 | 地盤高に乗じる倍率(感度実験・単位換算用) |
| f_user_routine | "" | 地形を加工するユーザールーチンの識別名(理想化実験用。[第I部](../users_guide.md#ユーザールーチン)) |

## 粗度(Manning 係数)

| パラメータ | 既定値 | 意味 |
|---|---|---|
| f_rntype | 0 | 0: 固定値 rn0、1: ファイル fn_rn、2: 土地利用から変換 |
| rn0 | 0.015 | 粗度係数固定値 |
| fn_rn | "" | 粗度係数分布ファイル |
| f_lusetype / fn_luse | 0 / "" | 土地利用データ(f_rntype=2 で使用) |
| lu2rn | — | 土地利用コード→粗度係数の対応表 |

土地利用からの変換(f_rntype=2)は、土地利用コードの分布 `fn_luse` と
対応表 `lu2rn` から粗度分布を作ります。

```
  f_rntype = 2
  f_lusetype = 1
  fn_luse = "landuse.txt"
  lu2rn(:,1) = 1, 0.06      ! コード 1(例: 田)→ n = 0.06
  lu2rn(:,2) = 2, 0.05      ! コード 2(例: 畑)→ n = 0.05
  lu2rn(:,3) = 5, 0.03      ! ...
```

## 領域マスク(計算対象の限定)

| パラメータ | 既定値 | 意味 |
|---|---|---|
| f_masktype | 0 | 0: 全域を計算、1: ファイル fn_mask、2: 海域マスクから自動生成 |
| fn_mask | "" | 領域マスクファイル(1: 対象、0: 対象外) |

マスク外のセルは**計算対象から完全に外れ、メモリも計算時間も消費
しません**(画面の `number of valid cells` で確認できます)。流域界で
切り出した広域計算ではメモリ・速度に直結するので、実地形では設定を
勧めます。f_masktype=2 は海域マスク(下記)から自動生成する簡便法で、
全陸域と「陸に接する1セル幅の海域」だけを対象化し、沖合の広い海域を
計算から外します。

## 海域マスク・河道マスク

| パラメータ | 既定値 | 意味 |
|---|---|---|
| fn_sw | "" | 海域マスク(1: 海域)。潮位・高潮・津波計算の海域指定(潮位の章 — 準備中) |
| fn_rw | "" | 河道マスク(1: 河道セル) |
| depth_rw / fn_depth_rw | 0 / "" | 河道セルの掘り込み深さ(一様値 / 分布。地形から掘り下げる) |
| rn0_rw | −1 | 河道セルの固定粗度(負値なら設定しない) |

河道マスクは「地形データの解像度では表現されない河道」を地形の
掘り込みと粗度の差し替えで表現する簡便法です。より精緻なサブグリッド
河道(断面形・河道幅・堤防)は fn_channel(河道の章 — 準備中)へ
段階的に移行できます。

## 家屋群の抵抗(市街地の氾濫)

| パラメータ | 既定値 | 意味 |
|---|---|---|
| fn_gv | "" | 家屋の空隙率(建蔽されていない割合)の分布 |
| fn_bb | "" | 家屋の平均寸法 (m) の分布 |
| min_gv / min_bb | 0.001 | それぞれの下限値(ゼロ割り防止) |

市街地の家屋群による貯留の減少と抗力を表現します(抗力係数などの
数値定数は &list_sysparam 側 — 浅水流計算の章(準備中))。

## ため池

| パラメータ | 既定値 | 意味 |
|---|---|---|
| fn_rscap | "" | ため池の限界貯留高 (m) の分布(値のあるセルがため池) |

地表水と別枠の貯留を持たせ、ポンプの取水源(構造物の章 — 準備中)
などに使います。水深は `f_out_hrs = 1` で `Hrs0001` として出力できます。

## 土層(地下水計算用)

| パラメータ | 既定値 | 意味 |
|---|---|---|
| f_sdtype | 0 | 0: 固定値 sd0、1: ファイル fn_sd |
| sd0 / fn_sd | 0 / "" | 土層厚 (m)(一様値 / 分布) |
| sy0 | 0.2 | 比湧水量(有効間隙率) |

土層厚を必要とする地下水モデル(Green-Ampt・側方流動)を選んだとき
だけ読まれ、メモリが確保されます(地下水の章 — 準備中)。

## 海岸堤防

| パラメータ | 既定値 | 意味 |
|---|---|---|
| fn_seaside | "" | 海側マスク(1: 海側扱い。fn_sw の海域は常に海側) |
| fn_seawall / seawall0 | "" / −9999 | 天端高の分布(陸側セル位置)/ 一律値(排他。一律値は海側に接する全陸セルへ自動付与=海岸線全周の壁) |
| f_seawall_datum | 2 | 天端高の基準(1: 自セル地盤からの高さ、2: 絶対標高) |
| f_seawall_mode | 0 | 水理モード(0: 越流のみ、1: フラップ=陸閘・水門、2: 強制排水) |

セルを太らせずに堤防をライン状の仮想壁面として表現します(河川堤防
fn_bank と同じ仕組みの海岸応用)。潮位ケースでは fn_seaside を省略
できますが(海域から自動判定)、**津波のような境界入射のケースでは
fn_seaside が必須**です。

## 例題・制約

- 最小構成: [tutorials/wave](../../tutorials/wave/)(格子のみ+ユーザールーチン)
- 実地形: [examples/chichibu](../../examples/chichibu/)、[examples/abukuma](../../examples/abukuma/)
- 分布ファイルの形式・座標系は[入出力](io.md)・[座標系](coordinates.md)の章を参照。
- 河道幅・堤防などのサブグリッド河道要素は fn_channel 側の設定です(河道の章 — 準備中)。
