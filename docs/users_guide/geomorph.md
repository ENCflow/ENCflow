...
- **設定の型**: `f_debris = 1`、`f_dbed = 0`、`f_dbres = 4`
  (μ = db_mu ≈ 0.15〜0.3、ξ = db_xi ≈ 1000〜3000 m/s² が雪崩の
  慣用域)、`f_dbstop = 1`(停止分を地形に固定)。発生区と破断深
  (雪の厚さ)は `fn_dbinit` の分布で与え、`db_reltime` で発生時刻を
  指定します。`fluv_porosity` は小さめ(例 0.1)にすると hs ≈ 雪厚の
  読みになります。`db_relsat` は小さめ(例 0.1)に —
  混合体モデルの近似上、少量の水をキャリアとして混ぜて流すためで、
  停止後にその水が残って流れる癖があります(量が小さければ実害は
  限定的)。急斜面なので `f_gravity_correction = 1` を推奨します。
- **できないこと(正直な限界)**: (1) 走路の雪の連行なし
  (f_dbed=0 は交換なし。破断量だけで流下するため、連行で成長する
  長走路の雪崩は過小評価)、(2) 煙型(粉雪)雪崩は対象外(空気中の
  サスペンション = 浅水近似の外)、(3) 発生の予測は不可(発生域・
  破断深は入力 — これは雪崩の専用モデルも同じで、実務では発生は
  シナリオとして与えるのが標準です)、(4) 衝撃圧の絶対値は要換算
  (F9999 は清水密度基準。雪崩の衝撃圧にするには流動雪密度
  200〜400 kg/m³ で読み替え)。
- Voellmy 定常等流の解析解一致(0.04%)は [test/volcano](../../test/volcano/)
  構成1(同じ構成)で恒常検定しています。雪崩実事例のランアウトとの
