...
| fn_gwc_outfall | "" | 吐口のオリフィス係数 Cd·A (m2) のマップ。値のあるセルが吐口(管路あり+海域セル隣接が必須) |
...
- 陽解法の安定条件が初期化時に検査されます。上限は疑似スロット
  gwc_slot_sy が支配するため、細かい格子で小さい slot_sy を使う場合は
  dt_gwflow を小さくするか slot_sy・gwc_eps_h を緩めます。
- gwc_leak_layer=1 は土層系(Green-Ampt か側方)の、=2 は f_gwlayer2=1 の
  有効化が前提です。
- **吐口**(fn_gwc_outfall): 隣接する海域セルの最低水位(潮位)を
  受け水頭とするオリフィス式の放流です。逆流防止フラップ内蔵
  (管路水頭 > 潮位のときのみ放流)、不圧・被圧を問いません(開放
  管端)。放流水は系外へ除去され(海は水位固定の受け皿)、S_total は
  放流分だけ減少します(累積放流体積は終了時に表示)。潮位より低い
  水頭の水は重力では吐けないため、海抜下の系の排水は排水ポンプの
  管路取水([構造物](structure.md)の f_pump_src=1)と組み合わせます。
...
使います。計算例のスクリプトは work/sewer_hybrid/gen_data.py
(幹線 D=1.0 m と枝管 D=0.4 m の換算の実演)を参照してください。
