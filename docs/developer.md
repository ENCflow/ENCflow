...
- **boundary_uvmn = エッジの uv/mn1 への強制条件の汎用適用点**
  ...
  - **開いた辺のモデルは2種で、統一しない(設計判断)**: 外側が
    ...
    - **長波放射(e_bc_radiation=2。津波向け)**:
      `uve = √(g/he)·(z+h − η_ref)`(η_ref は辺ごとの基準水位
      bc_eta_*)。静水では厳密にフラックスゼロ、波の水位偏差だけを
      透過し、引き波の流入(負値)も許す。乾いたセルはゼロ
      (境界からの再湿潤はしない簡易化)。検証: 静水 30 秒で S が
      ビット不変、波の透過で残留 h_max 偏差 ~2e-4(120 秒後)。
