...
- **区間流入の配分モード inflow_dist(2026-08-03)**: 0=開口幅で均等
  ...
  重みは前ステップの s%h で毎ステップ更新。dist=1,2 の有効条件は
  (i) wet(h>=dd)面エントリが過半、(ii) 最深セルの配分流量が限界流量
  以下(qw_max <= h_max·sqrt(g·h_max))の両方で、どちらか欠けると
  均等按分に落ちる。(ii) は実発生の発散への安定性ガード: 薄膜断面で
  wet 率が閾値近傍まで痩せると重みの分母(通水能)が小さくなり、
  最深セルへの流量集中(dh/dt ~ 0.1 m/s)が薄膜上のダム崩壊流を
  起こして数ステップで発散する。面流速自体は限界水深の床で有界の
  ため、ガードは面ではなく「断面が Q を常流で受け入れられるか」で
  判定する。MPI: 重みの水深ベクトルは「所有ランクだけが埋める
