...
- 全国スケール計算のメモリ: 場の配列は 1/nproc でスケールするようになった。
  次の床は「静的地理データの全ランク全域保持」(g%z, g%x, rn 等で全国
  100m ≒ 数 GB/ランク)。対処(静的データの帯化)は通信不要で機械的だが、
  全域前提の初期化・record・fill_depression の再点検が要るため、実測で
  必要になってから。
...
- **帯確保の配列を手続きに渡す場合、ダミーは assumed-shape+下限指定を
  ...
  静的・全域保持の配列(g%z, g%x 等)は明示形状でよいが、混在する署名では
  band 側に合わせて全引数を assumed-shape にするのが安全。
