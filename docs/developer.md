...
- **静的データは「初期化は全域・時間ループは帯」の二相で保持する**
  (下記「静的データの帯化」参照)。初期化中は全ランクが全域を冗長構築する
  ため、境界・湧き出し・プローブの位置解決は従来どおり手元の全域情報で
  完結する(冗長計算=配布機構、という原則は不変)。
...
- **静的データの帯化(方式a: 2点縮小)実装・検証済み(2026-xx)**:
  ...
  - ゾーン1(m_geoinfo_init 内)= 全配列が全域。地理データの読み込み・
    導出(adjust_rw の河道掘り込み等)・user フック・窓計算・n_valcells
    カウントの住所。**係数を使う全域前処理は必ずここに書く**。
  - par_decomp_init → m_geoinfo_shrink_coeffs: 物性係数
    (rn, gv, bb, lm, rsh, lu)を帯+ハロに縮小。
  ...
  - 残る初期化中の一過性ピーク(全ランクが全域一式を保持)は未対策。
    単一ノードの全国初期化が OOM したら、読み込みと縮小の交互化
    (結合処理 adjust_rw/lu2rn を縫う読み順の再編)を検討する。
