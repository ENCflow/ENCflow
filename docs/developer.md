...
- **第二段(確保の縮小 jsh/jeh = 担当帯±nhalo)は未着手**。対象は TODO
  マーク済み: user_initial / fill_depression の全域一時配列+切り出し、
  prtype3 読み込みの切り出し。全国スケール計算はメモリ律速のため第二段が
  先に必要になる(100m 全国 ≒ 10⁸ セル × 全域確保 = 数十 GB/ランク)。
