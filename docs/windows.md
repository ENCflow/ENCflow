...
- エクスプローラーのアドレス欄に `\\wsl$` と打つと、WSL の中の
  ファイルが**ふつうのフォルダとして**見えます。ただし最初に見えるのは
  Linux 全体のルートなので、自分のファイルは
  `Ubuntu` → `home` → `自分のユーザー名`(WSL の初回起動時に決めた名前)
  とたどった先にあります。上の手順どおりなら ENCflow は
  `\\wsl$\Ubuntu\home\ユーザー名\ENCflow` です。よく使うので、
  この場所を**クイックアクセスにピン留め**しておくと便利です。
  パラメータファイルの編集はメモ帳や VS Code で、結果のテキスト・CSV は
  Excel で、GeoTIFF は GIS ソフトでそのまま開けます。
