...
8. **経緯度(度単位)の bil にも対応する(2026-08-01 拡張)**。
   ...
   - CRS が決まらない(namelist の epsg 未指定・ファイルにも CRS なし)
     経緯度格子は **WGS84(EPSG:4326)を仮定**し、その旨を画面表示する
     (2026-08-01 追加。概算検査が既に WGS84 前提であることと一貫。
     JGD2000/2011 との差は cm 級。GeoTIFF 出力に CRS が付く)。
     投影座標系は系番号を推測できないため従来どおり CRS なしで出力する。
